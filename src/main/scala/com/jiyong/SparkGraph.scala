...
object SparkGraph {
  def main(args: Array[String]): Unit = {
    ...
    //设置运行环境
    val conf = new SparkConf().setAppName("GraphxTest").setMaster("local[2]")
    ...
    println("属性演示")
    println("*******************************************")
    println("找出图中年龄大于30的定点：")
    graph.vertices.filter(t => {
      t._2._2 > 30
    }).collect().foreach(t => {
      println(s"${t._2._1} is ${t._2._2}")
    })
    graph.triplets.foreach(t => {
      println(s"triplet:${t.srcId},${t.srcAttr},${t.dstId},${t.dstAttr},${t.attr}")
    })
    ...
    println("*************************************************************")
    println("转换操作")
    println("定点的转换操作，顶点 age + 10:")
    graph.vertices.map(t => {
      (t._1,(t._2._1,t._2._2 + 10))
    }).collect().foreach(t => {
      println(s"${t._2._1} is ${t._2._2}")
    })
